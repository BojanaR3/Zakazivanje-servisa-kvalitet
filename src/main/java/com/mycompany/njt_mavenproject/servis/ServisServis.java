...
public class ServisServis {
     ...
     * @throws EntityUpdateException ako servis sa datim ID-jem ne postoji
     */
    @Transactional
    public ServisDto update(ServisDto dto) {
        Servis s;
        try { s = repo.findById(dto.getId()); }
        catch (EntityNotFoundException e) { throw new EntityUpdateException("Servis ne postoji: " + dto.getId()); }
